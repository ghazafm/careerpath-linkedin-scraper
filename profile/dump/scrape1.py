...
def extract_experience(driver, profile_url):
    ...
    experience_list = get_objects(driver, By.CLASS_NAME, "//li[contains(@class, 'pvs-list__paged-list-item artdeco-list__item')]")
    
    # Iterate over each experience item
    for experience in experience_list:
        try:
            # Extract job title
            job_title = extract_element_text(experience, By.XPATH, ".//span[@aria-hidden='true'][1]")

            # Extract company name
            company_name = extract_element_text(experience, By.XPATH,  ".//div[contains(@class, 'hoverable-link-text')]//span[@aria-hidden='true']")

            # Extract duration
            duration = extract_element_text(experience, By.XPATH,  ".//span[@aria-hidden='true'][contains(text(), '·')]")

            # Extract location
            location = extract_element_text(experience, By.XPATH, ".//span[@aria-hidden='true'][contains(text(), ',')]")
...
# def extract_experience1(driver, profile_url):
#     driver.get(profile_url)
#     scroll_and_load(driver)

#     # Find the experience list container (companies)
#     experience_list = driver.find_elements(By.XPATH, "//li[contains(@class, 'pvs-list__paged-list-item artdeco-list__item')]")

#     # Iterate over each company in the experience section
#     for experience in experience_list:
#         try:
#             # Extract company name (Ensure no duplicates by using only aria-hidden="true")
#             company_name = extract_element_text(
#                 experience, By.XPATH, ".//div[contains(@class, 'hoverable-link-text')]//span[@aria-hidden='true']"
#             )
            
#             print(f'company: {company_name}')
            
#             # Extract job positions within the company (nested loop for positions)
#             position_list = experience.find_elements(By.XPATH, ".//li[contains(@class, 'pvs-list__paged-list-item')]")
            
#             for position in position_list:
#                 # Extract job title (Ensure no duplicates by using aria-hidden="true")
#                 job_title = extract_element_text(
#                     position, By.XPATH, ".//span[@aria-hidden='true'][1]"  # Ensuring that you pick the right visible element
#                 )
#                 print(f'job: {job_title}')
                
#                 # Extract duration and location (use aria-hidden to avoid duplicate text)
#                 duration = extract_element_text(
#                     position, By.XPATH, ".//span[@aria-hidden='true'][contains(text(), '·')]"  # Typically duration contains '·'
#                 )
#                 print(f'duration: {duration}')
                
#                 location = extract_element_text(
#                     position, By.XPATH, ".//span[@aria-hidden='true'][contains(text(), ',')]"  # Location usually has commas
#                 )
...
#                 experience_data = {
#                     "company_name": company_name,
#                     "job_title": job_title.strip(),  # Clean up any trailing whitespaces
...
def extract_education(driver, profile_url):
    ...
    education_entries = get_objects(
        driver,
        By.XPATH,
        "//section[contains(@class, 'artdeco-card') and contains(., 'Education')]//ul/li[contains(@class, 'artdeco-list__item')]",
    )
    ...
    for entry in education_entries:
        try:
            ...
            university_name = extract_element_text(
                entry,
                By.XPATH,
                ".//a[contains(@href, 'company')]//div[contains(@class,'hoverable-link-text')]",
            )
...
def extract_certificate(driver, profile_url):
    ...
    certifications = get_objects(
        driver,
        By.XPATH,
        "//ul[contains(@class, 'GwDizhdSHYPkIbgBuMBRdRJgYmpMKyoUqjqU')]/li",
    )
    ...
    for cert in certifications:
        ...
        try:
            issuer = extract_element_text(
                cert, By.XPATH, ".//span[@aria-hidden='true'][2]"
            )
        ...
        try:
            date_issued = extract_element_text(
                cert,
                By.XPATH,
        ...
        try:
            credential_link = extract_element_attribute(
                cert, By.XPATH, ".//a[contains(@aria-label, 'Show credential')]", "href"
            )
...
def extract_volunteering(driver, profile_url):
    ...
    volunteering_entries = get_objects(
        driver,
        By.XPATH,
        "//section[contains(@class, 'artdeco-card') and contains(., 'Volunteer experience')]//ul/li[contains(@class, 'artdeco-list__item')]",
    )
    ...
    for entry in volunteering_entries:
        try:
            ...
            cause = extract_element_text(
                entry, By.XPATH, ".//span[@class='t-14 t-normal t-black--light']"
            )
...
def extract_project(driver, profile_url):
    ...
    projects_element = get_objects(
        driver, By.XPATH, "//ul[@class='GwDizhdSHYPkIbgBuMBRdRJgYmpMKyoUqjqU ']/li"
    )
    ...
    for project in projects_element:
        ...
        try:
            description = extract_element_text(
                project,
                By.XPATH,
                ".//div[@class='t-14 t-normal t-black']/span[@aria-hidden='true']",
            )
...
def extract_skill(driver, profile_url):
    ...
    skills_element = get_objects(
        driver, By.XPATH, "//li[contains(@class, 'pvs-list__paged-list-item')]"
    )
...
def extract_honor(driver, profile_url):
    ...
    for honor in honors_elements:
        ...
        issuer_date = extract_element_text(
            honor, By.XPATH, ".//span[contains(@class, 't-14 t-normal')]"
        )

        # Extract the associated institution if available
        try:
            associated_institution = extract_element_text(
                honor, By.XPATH, ".//span[contains(text(), 'Associated with')]"
            )
        except:
...
def extract_organizations(driver, profile_url):
    ...
    organization_entries = get_objects(
        driver,
        By.XPATH,
        "//section[contains(@class, 'artdeco-card') and contains(., 'Organizations')]//ul/li[contains(@class, 'artdeco-list__item')]",
    )
    ...
    for entry in organization_entries:
        try:
            ...
            description = extract_element_text(
                entry, By.XPATH, ".//div[contains(@class, 't-14 t-normal t-black')]"
            )
...
def scrape_profile(driver, profile_url, visited_profiles):
    ...
    profile_data['experience'] = extract_experience(driver, profile_url + 'details/experience/')
