...
def login(driver):
    ...
    # time.sleep(100)
    password_element = get_object(driver, By.XPATH, "//input[@id='password' and @name='session_password' and @type='password']")
...
def extract_many_element_text(driver, by, element, timeout=timeout):
    try:
        return get_objects(driver, by, element, timeout=timeout).text.strip()
    except NoSuchElementException:
        return "Not available"
...
def extract_many_element_attribute(driver, by,  element, attribute):
    try:
        return get_objects(driver, by, element, timeout=timeout).get_attribute(attribute)
    except NoSuchElementException:
        return "Not available"
...
def start_chrome_with_debug():
    ...
    chrome_options.add_experimental_option("debuggerAddress", "127.0.0.1:9222")  # Connect to the remote-debugging Chrome
